...
for freqOffset = 1:length(freqList)
    for ampOffset = 1:length(ampList)
        disp(['Frequency ', num2str(freqList(freqOffset)), ' (', num2str(freqOffset), ' of ' , num2str(length(freqList)), '), Amplitude ', num2str(ampList(ampOffset)), ' (', num2str(ampOffet), ' of ' , num2str(length(ampList)), ')']);
        TT.ResetFilters
