swepNum = 1;
preSpikeWindowPlot = 7;
postSpikeWindowPlot = 22;

sampleRate = 24414.06;

preSnipWindowPlot = 0;
postSnipWindowPlot = 30;
...
snipLength = length(thisStimSnipData{swepNum}(:,1));
...
for spikeToPlot = 1:length(thisStimSpikeTimesSingle)
    if spikeToPlot == 1
        ...
        disp(['Spike ', num2str(spikeToPlot), ' occurred at time ', num2str(thisStimSpikeTimesSingle(spikeToPlot)), ': ', num2str((thisStimSpikeTimesSingle(spikeToPlot) - thisStimSpikeTimesSingle(spikeToPlot-1))*sampleRate), ' samples after the previous spike']);
    end
end
